...
public class VIA6522_BC extends VIA6522 implements Observer {
    ...
    public int readRegister(final int register) {
        switch (register) {
            case PRB:
                return (short) ((super.readRegister(register) & 0x1a) | this.c1541.getID() << 5 | (this.iecBus.getSignal(IECBus.ATN) ? SB_ATN_IN : 0) | (this.iecBus.getSignal(IECBus.CLK) ? SB_CLK_IN : 0) | (this.iecBus.getSignal(IECBus.DATA) ? SB_DATA_IN : 0));
