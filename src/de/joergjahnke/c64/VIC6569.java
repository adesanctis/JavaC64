...
public class VIC6569 extends DefaultObservable implements IOChip, Observer, Serializable {
     ...
     */
    protected final short[] registers = new short[0x40];
    /**
    ...
    public VIC6569(final C64 c64) {
        ...
        for (int i = 0; i < sprites.length; ++i) {
            this.sprites[i] = new Sprite(this.memory);
        }
    }
    ...
    private final void writeSpriteSpriteCollisions(final short collisions) {
        ...
        // only latch on first occasion
        if ((this.irqFlags & IRQ_SPRITE_SPRITE_COLLISION) == 0 && this.registers[ 0x1e] != 0) {
    ...
    private final void writeSpriteBitmapCollisions(final short collisions) {
        ...
        // only latch on first occasion
        if ((this.irqFlags & IRQ_SPRITE_BACKGROUND_COLLISION) == 0 && this.registers[ 0x1f] != 0) {
    ...
    private final int getSpriteDataPointer(final int spriteNo) {
        return this.videoMemBase + readByte(this.videoMatrixBase + 0x03f8 + spriteNo) * 64;
    }
    ...
    private void doVideoMatrixAccess() {
        ...
        if (this.isDisplayActive) {
            ...
            this.videoMatrixData[vmli_] = readByte(this.videoMatrixBase + vc_);
    ...
    private void doGraphicsAccess() {
        ...
        if (this.isDisplayActive) {
            ...
            // read pixel data
            this.graphicsData = readByte(address);
        } else {
     ...
     */
    private short readByte(final int adr) {
    ...
    public final int readRegister(final int register) {
        switch (register) {
            // control register 1
            case 0x11: {
                short result = this.registers[register];
            ...
            // Raster Counter
            case 0x12:
                return (short) (this.rasterY & 0xff);

            // Interrupt Pending Register
            case 0x19:
                return (short) (this.irqFlags | 0x70);

            // Interrupt Mask Register
            case 0x1a:
                return (short) (this.irqMask | 0xf0);

            // clear sprite collision registers after read
            case 0x1e:
            case 0x1f: {
                final short result = this.registers[register];

                this.registers[register] = (short) 0;
                return result;
            }

            // for addresses < $20 read from register directly, when < $2f set bits of high nibble to 1, for >= $2f return $ff
            default:
                return register < 0x20 ? this.registers[register] : register < 0x2f ? (short) (this.registers[register] | 0xf0) : 0xff;
        }
    }

    public final void writeRegister(final int register, final short data) {
        ...
        switch (register) {
            ...
            case 0x12:
                ...
                if (this.rasterYIRQ != readRasterLineIRQ()) {
                    ...
                    if (this.rasterY == this.rasterYIRQ && (this.irqFlags & 0x01) == 0) {
