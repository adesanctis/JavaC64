...
public class VIA6522_DC extends VIA6522 implements Observer {
    ...
    protected void rotateDisk() {
        ...
        if (!this.isWriteMode && this.gcrPos >= 0) {
            ...
            this.registers[PRA] = (short) (this.gcrSector[this.gcrPos] & 0xff);
        }
    ...
    private void readSector() {
        ...
        this.gcrSector = new short[GCR_SECTOR_SIZE];
        this.gcrPos = 0;
        this.shiftGCR = 0;
        this.bytesGCR = 0;
    ...
    private void writeGCRByte(final int data) {
        ...
        if (++this.bytesGCR >= 4) {
            ...
            for (int shift = 32; shift >= 0; shift -= 8) {
                this.gcrSector[this.gcrPos++] = (short) ((this.shiftGCR >> shift) & 0xff);
            }
    ...
    public int readRegister(final int register) {
        switch (register) {
            case PRB: {
                final short result = (short) ((super.readRegister(register) & (0xff - PB_WRITE_PROTECTED - PB_SYNC)) | (isWriteProtect() ? 0 : PB_WRITE_PROTECTED) | (isSync() ? 0 : PB_SYNC));
            ...
            case PRA:
            case PRA2: {
                final short result = this.registers[PRA];
    ...
    public void serialize(final DataOutputStream out) throws IOException {
        ...
        for (int i = 0; i < size; ++i) {
            out.writeShort(this.gcrSector[i]);
        }
    ...
    public void deserialize(final DataInputStream in) throws IOException {
        ...
        if (size > 0) {
            this.gcrSector = new short[size];
            for (int i = 0; i < size; ++i) {
                this.gcrSector[i] = in.readShort();
            }
        } else {
            this.gcrSector = null;
        }
