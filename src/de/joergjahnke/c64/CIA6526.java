...
public abstract class CIA6526 extends DefaultObservable implements IOChip, Serializable {
     ...
     * Timer A
     */
    private Timer timerA = new Timer(getClass().getName() + " (Timer A)");
    /**
     * Timer B
     */
    private Timer timerB = new Timer(getClass().getName() + " (Timer B)");
    /**
     ...
     * IRQ mask
     */
    protected short irqMask = 0;
    /**
     * Alarm time 1/10 seconds
     */
    private short alarmTen = 0;
    /**
     * Alarm time seconds
     */
    private short alarmSec = 0;
    /**
     * Alarm time minutes
     */
    private short alarmMin = 0;
    /**
     * Alarm time hours
     */
    private short alarmHrs = 0;
    /**
     * Time of day time 1/10 seconds
     */
    private short todTen = 0;
    /**
     * Time of day time seconds
     */
    private short todSec = 0;
    /**
     * Time of day time minutes
     */
    private short todMin = 0;
    /**
     * Time of day time hours
     */
    private short todHrs = 0;
    /**
     ...
     */
    protected final short[] registers = new short[0x10];
    ...
    public int readRegister(final int register) {
        switch (register) {
            case PRA:
                return (short) (this.registers[register] | ~this.registers[DDRA]);
            case PRB:
                return (short) (this.registers[register] | ~this.registers[DDRB]);

            case TIMER_A_LOW:
                return (short) ((this.timerA.isActive ? Math.max(0, this.timerA.nextTrigger - this.cpu.getCycles()) : this.timerA.timeLeft) & 0xff);
            case TIMER_A_HIGH:
                return (short) ((this.timerA.isActive ? Math.max(0, this.timerA.nextTrigger - this.cpu.getCycles()) : this.timerA.timeLeft) >> 8);
            case TIMER_B_LOW:
                return (short) ((this.timerB.isActive ? Math.max(0, this.timerB.nextTrigger - this.cpu.getCycles()) : this.timerB.timeLeft) & 0xff);
            case TIMER_B_HIGH:
                return (short) ((this.timerB.isActive ? Math.max(0, this.timerB.nextTrigger - this.cpu.getCycles()) : this.timerB.timeLeft) >> 8);
    ...
    public void writeRegister(final int register, final short data) {
        switch (register) {
            // only write to bits allowed by the data direction registers
            case PRA:
                this.registers[register] = (short) ((this.registers[register] & ~this.registers[DDRA]) | (data & this.registers[DDRA]));
                break;
            case PRB:
                this.registers[register] = (short) ((this.registers[register] & ~this.registers[DDRB]) | (data & this.registers[DDRB]));
                break;
    ...
    public void serialize(final DataOutputStream out) throws IOException {
        out.writeShort(this.alarmHrs);
        out.writeShort(this.alarmMin);
        out.writeShort(this.alarmSec);
        out.writeShort(this.alarmTen);
        out.writeShort(this.crb);
        out.writeShort(this.icr);
        out.writeShort(this.irqMask);
        out.writeLong(this.nextDayTimeUpdate);
        out.writeLong(this.nextUpdate);
        out.writeInt(this.offset);
        out.writeShort(this.todHrs);
        out.writeShort(this.todMin);
        out.writeShort(this.todSec);
        out.writeShort(this.todTen);
    ...
    public void deserialize(final DataInputStream in) throws IOException {
        this.alarmHrs = in.readShort();
        this.alarmMin = in.readShort();
        this.alarmSec = in.readShort();
        this.alarmTen = in.readShort();
        this.crb = in.readShort();
        this.icr = in.readShort();
        this.irqMask = in.readShort();
        this.nextDayTimeUpdate = in.readLong();
        this.nextUpdate = in.readLong();
        this.offset = in.readInt();
        this.todHrs = in.readShort();
        this.todMin = in.readShort();
        this.todSec = in.readShort();
        this.todTen = in.readShort();
    ...
    class Timer implements Serializable {
         ...
         */
        public void applyControlRegister(final short data) {
